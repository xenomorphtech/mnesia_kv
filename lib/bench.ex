defmodule MnesiaKV.Bench do
  ...
  def write_to_file_unsafe(threads \\ 4, file \\ "/tmp/dump") do
    ...
    Enum.each(1..threads, fn _idx ->
      :erlang.spawn_opt(
        fn ->
          {:ok, file} = :file.open(file, [:write, :raw, :binary, {:delayed_write, 524_288, 8}])

          {took, _} =
            :timer.tc(fn ->
              Enum.each(1..100_000, fn _key ->
                key = :rand.uniform(10000)

                term = {
                  key,
                  case :ets.lookup(Bench, key) do
                    [] -> %{field: :rand.uniform(1_000_000)}
                    [{_, old}] -> Map.merge(old, %{field: :rand.uniform(100_000)})
                  end
                }
          ...
          {took, _} =
            :timer.tc(fn ->
              Enum.each(1..100_000, fn _key ->
                ...
                map =
                  case ignore_warning.dirty_read(BenchMnesia, key) do
                    [] -> %{field: :rand.uniform(1_000_000)}
                    [{_, _, old}] -> Map.merge(old, %{field: :rand.uniform(100_000)})
                  end
